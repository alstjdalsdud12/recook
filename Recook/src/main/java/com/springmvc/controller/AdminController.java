...
public class AdminController {
    ...
    public String adminPage(Model model) {
        ...
        // 최근 문의 5개
        List<Inquiry> recentInquiryList = inquiryDAO.selectAll().stream()
            .limit(5)
    ...
    public String submitReply(@RequestParam(name = "i_no") int i_no, @RequestParam(name = "i_reply") String i_reply, HttpSession session) {
        String m_role = (String) session.getAttribute("m_role");
        if (m_role == null || !m_role.equals("admin")) {
 ...
 // 레시피 삭제 + 알림 전송
    ...
    public String recipeDeleteWithNotify(
